...
bool ArmVisualizer::loadSvgModels_() {
    const QString arm_path = QStringLiteral(":/models/arm_segment.svg");
    ...
    const QString base_path = QStringLiteral(":/models/base_segment.svg");
...
void ArmVisualizer::applyTheme(const davinci_arm::ui::style::ThemeSpec& spec) {
    ref_color_ = spec.ref_color;
...
void ArmVisualizer::paintEvent(QPaintEvent* /*event*/) {
    ...
    if (show_ref_) {
        ...
        painter.drawText(
            10,
            y,
            QStringLiteral("Ref: %1\u00B0").arg(static_cast<double>(ref_angle_ * 180.0 / M_PI), 0, 'f', 1));
        y += 16;
    }
    if (show_real_) {
        ...
        painter.drawText(
            10,
            y,
            QStringLiteral("Real: %1\u00B0").arg(static_cast<double>(real_angle_ * 180.0 / M_PI), 0, 'f', 1));
        y += 16;
    }
    if (show_sim_) {
        ...
        painter.drawText(
            10,
            y,
            QStringLiteral("Sim: %1\u00B0").arg(static_cast<double>(sim_angle_ * 180.0 / M_PI), 0, 'f', 1));
    }
}
...
void ArmVisualizer::drawBase_(QPainter& painter, const QPointF& pivot) {
    ...
    if (base_renderer_ && base_renderer_->isValid()) {
        ...
        const QRectF base_rect(pivot.x() - offset_x, pivot.y() - offset_y, bw, bh);
        ...
        const QRectF base_rect(pivot.x() - bw / 2.0, pivot.y() - bh / 2.0, bw, bh);
...
void ArmVisualizer::drawArm_(QPainter& painter,
    ...
    if (arm_renderer_ && arm_renderer_->isValid()) {
        ...
        const QRectF arm_rect(-offset_x, -offset_y, aw, ah);
        ...
        const int alpha_value = static_cast<int>(clamped_opacity * 255.0);

        for (int y = 0; y < svg_image.height(); ++y) {
            auto* line = reinterpret_cast<QRgb*>(svg_image.scanLine(y));
            for (int x = 0; x < svg_image.width(); ++x) {
                ...
                if (gray < 50) {
                    ...
                    const int adjusted_alpha = static_cast<int>((alpha * clamped_opacity * 255.0) / 255.0);
        ...
        const QRectF arm_rect(0, -half_w, length, aw);
        ...
        if (shadow) {
            ...
            if (clamped_opacity > 0.5) {
                QColor highlight = color.lighter(140);
                highlight.setAlpha(static_cast<int>(80 * clamped_opacity));
                painter.setBrush(highlight);
                const QRectF hi_rect(length * 0.15, -half_w * 0.5, length * 0.15, aw * 0.5);
